...
begin
  ...
  where id = p_after_gameweek_id
  for update;

  if anchor.id is null then
    raise exception 'Anchor gameweek not found';
  end if;

  if p_opens_at is null or p_locks_at is null or p_opens_at >= p_locks_at then
    raise exception 'One-off gameweek must open before its deadline';
  end if;

  if p_selection_rule_mode not in ('exact_time', 'any_kickoff') then
    raise exception 'Invalid selection rule mode';
  end if;

  if p_selection_weekday < 1 or p_selection_weekday > 7 then
  ...
  from public.gameweeks
  where season_id = anchor.season_id;

  -- Two-stage shift avoids the unique(season_id, number) constraint colliding.
  update public.gameweeks
  set number = number + shift_by
  where season_id = anchor.season_id
    and number > anchor.number;

  update public.gameweeks
  set number = number - shift_by + 1
  where season_id = anchor.season_id
    and number > anchor.number + shift_by;
