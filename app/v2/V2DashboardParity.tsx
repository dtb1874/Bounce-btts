...
export default function V2DashboardParity(props:Props){
 ...
 const gridStyle={"--weeks":formWeeks.length} as CSSProperties;
 return <section className={styles.shell} aria-label="V1 feature parity tools">
   ...
   <section className={styles.section} id="v2-current-form"><div className={styles.head}><div><span>RECENT FORM</span><h3>{formRange}-Week Form</h3></div><div className={styles.headActions}><select aria-label="Form range" value={formRange} onChange={event=>setFormRange(Number(event.target.value) as 6|12|18)}><option value={6}>6 weeks</option><option value={12}>12 weeks</option><option value={18}>18 weeks</option></select><DataShareButton compact title={`${formRange}-Week Form`} subtitle={`Season ${props.seasonLabel} · through GW ${selectedNumber}`} columns={["Player",...formWeeks.map(g=>`GW${g.number}`),"Total"]} rows={shareRows} fileName={`bounce-form-${formRange}w.jpg`} label={`Share ${formRange}-week form`}/></div></div><div className={styles.formWrap}><div className={styles.formGrid}><div className={`${styles.formRow} ${styles.formHeader}`} style={gridStyle}><strong>Player</strong>{formWeeks.map(g=><span key={g.id}>GW{g.number}</span>)}<b>Total</b></div>{formRows.map(row=><div className={styles.formRow} style={gridStyle} key={row.name}><strong>{row.name}</strong>{row.values.map((value,index)=><span key={`${row.name}-${formWeeks[index]?.id}`} className={value===3?styles.good:value===1?styles.ok:value!=null&&value<0?styles.bad:styles.muted}>{value==null?"—":value>0?`+${value}`:value}</span>)}<b>{row.total>0?`+${row.total}`:row.total}</b></div>)}</div></div></section>
