...
import V2ShotPerformance from "./V2ShotPerformance";
...
function odds(value: number | null | undefined) { return value == null ? "—" : `${value.toFixed(2)}/1`; }
...
export default function V2StatCentre({ seasonLabel, profiles, gameweeks, fixtures, predictions, adjustments, standings, myId, entryFee }: Props) {
  ...
  useEffect(() => {
    ...
    void fetch("/api/member-portraits", { cache: "force-cache" })
      ...
      .then((data) => { if (cancelled) return; const next: Record<string, string> = {}; for (const row of data.portraits ?? []) if (row.id && row.portraitUrl) next[row.id] = row.portraitUrl; setPortraits(next); })
      .catch(() => undefined);
  ...
  const canonical = useMemo(() => calculateLeagueStats({
    ...
    predictions,
    adjustments,
    fixtures: fixtures.map((row) => ({ ...row, odds_fractional: row.odds_deadline_fractional ?? row.odds_fractional })),
  ...
  const creature = canonical.headline.creatureLeaders.length ? canonical.headline.creatureLeaders.map((row) => `${row.name} · ${row.team}`).join(" / ") : "—";
  const mostPickedTeamRecord = canonical.seasonFacts.find((fact) => fact.label === "MOST PICKED TEAM" || fact.label === "MOST PICKED TEAMS");
  const tabs: Array<{ id: Tab; label: string }> = [{ id: "league", label: "League" }, { id: "players", label: "Players" }, { id: "form", label: "Form & Trends" }, { id: "records", label: "Records" }];

  function clearPortrait(id: string) { setPortraits((current) => { const next = { ...current }; delete next[id]; return next; }); }

  return <main className={styles.page}>
    <header className={styles.hero}>
      <div className={styles.heroTitle}><span>SEASON {seasonLabel}</span><h1>Stat Centre</h1><p>League intelligence, player tendencies and season records.</p></div>
    ...
    <nav className={styles.tabs} aria-label="Stat Centre sections">{tabs.map((item) => <button type="button" key={item.id} className={tab === item.id ? styles.activeTab : ""} onClick={() => setTab(item.id)}>{item.label}</button>)}</nav>

    {tab === "league" ? <section className={styles.section}>
      <header className={styles.sectionHeading}><span>SEASON {seasonLabel}</span><h2>League Overview</h2></header>
      <div className={styles.leagueLeadLine}><div><span>LEAGUE PULSE</span><strong>{canonical.headline.leagueLeader?.name ?? "No leader yet"}</strong><p>{canonical.headline.leagueLeader ? `${canonical.headline.leagueLeader.points} points at the top` : "Waiting for the first scored gameweek"}</p></div><div className={styles.leaguePulseStats}><div><span>SEASON POT</span><b>£{prizePot.toFixed(0)}</b></div><div><span>STRIKE RATE</span><b>{pct(canonical.headline.leagueStrikeRate)}</b></div><div><span>BTTS WINS</span><b>{canonical.headline.bttsWins}</b></div></div></div>
    ...
    {tab === "players" ? <section className={styles.section}>
      ...
      <V2ShotPerformance profiles={activeProfiles} predictions={predictions} fixtures={fixtures}/>
    </section> : null}
