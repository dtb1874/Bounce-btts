...
type Props = { seasonLabel: string; gameweeks: Gameweek[]; predictions: LeagueStatsPrediction[]; fixtures: LeagueStatsFixture[] };
...
export default function SweepTracker(props:Props){
  ...
  return createPortal(<section className={`${styles.shell} ${sharing?styles.sharing:""}`}><div className={styles.head}><div><span>WEEKLY NEAR-MISS TRACKER</span><h3>Goals Away From The Sweep</h3><p>How many extra goals were needed for every selected match to land BTTS.</p></div><button className="dataShareButton shareCompactWhatsApp" type="button" onClick={shareAnimation}>{sharing?"Creating…":"Share tracker"}</button></div>{points.length?<><svg className={styles.chart} viewBox={`0 0 ${viewWidth} ${viewHeight}`} role="img" aria-label="Goals away from all BTTS wins by gameweek">{Array.from({length:maxValue+1},(_,value)=>{const y=bottom-(value/maxValue)*(bottom-top);return <g key={value}><line className={styles.grid} x1={left} x2={right} y1={y} y2={y}/><text className={styles.axisText} x={8} y={y+3}>{value}</text></g>})}{coords.map((point)=><text key={`gw-${point.gameweek}`} className={styles.axisText} x={point.x} y={bottom+24} textAnchor="middle">GW {point.gameweek}</text>)}<polyline className={styles.line} points={polyline}/>{coords.map((point)=><g key={point.gameweek}><circle className={styles.dot} cx={point.x} cy={point.y} r={5}/><text className={styles.value} x={point.x} y={point.y-10} textAnchor="middle">{point.goalsAway}</text></g>)}</svg><div className={styles.summary}><span>0 = complete BTTS sweep</span><strong>{latest?`Latest: GW ${latest.gameweek} · ${latest.goalsAway} away`:""}</strong></div></>:<div className={styles.summary}><span>The tracker starts once a gameweek is fully scored.</span></div>}</section>,target)}
