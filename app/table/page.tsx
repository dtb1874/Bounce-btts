...
export default async function PublicTablePage(){
  ...
  const [{data:settings},{data:season},{data:profiles}]=await Promise.all([
  ...
  const {data:gameweeks}=season?.id?await admin.from("gameweeks").select("id,number,status,opens_at,locks_at").eq("season_id",season.id).order("number"):{data:[]};
  ...
  return <main className="spectatorPage">
    ...
    <section className="spectatorStatus"><article><span>GAMEWEEK</span><strong>GW {current?.number??"—"}</strong><small>{current?.status?.toUpperCase()??"—"}</small></article><article><span>LIVE NOW</span><strong>{liveCount}</strong><small>selected matches</small></article><article><span>LEADER</span><strong>{rows[0]?.name??"—"}</strong><small>{rows[0]?`${rows[0].points} pts`:"No scores"}</small></article><article><span>SEASON POT</span><strong>£{((profiles?.length??0)*Number(settings?.entry_fee??20)).toFixed(0)}</strong><small>{profiles?.length??0} players</small></article></section>
    <section className="spectatorSection"><div className="spectatorHeading"><div><span>GAMEWEEK {current?.number??"—"}</span><h3>Everyone's picks</h3><p>Scores and BTTS state shown from the latest league data.</p></div><b>{currentPreds.length}/{profiles?.length??0} picks</b></div><div className="spectatorPicks">{picks.map(({profile,p,f})=>{const state=f&&p?outcome(f,p):["AWAITING PICK","neutral"];return <article key={profile.id} className={`spectatorPick ${state[1]}`}><div className="spectatorPlayer"><span>{profile.display_name.split(/\s+/).map(x=>x[0]).join("").slice(0,2)}</span><strong>{profile.display_name}</strong></div>{f?<><div className="spectatorFixture"><strong>{f.home_team} <em>v</em> {f.away_team}</strong><small>{comp(f)} · BTTS {f.odds_fractional??"—"}</small></div><div className="spectatorScore"><strong>{f.home_score==null?"—":`${f.home_score}-${f.away_score}`}</strong><small>{statusText(f)}</small></div></>:<div className="spectatorFixture"><strong>Awaiting selection</strong><small>Nothing submitted yet</small></div>}<div className="spectatorOutcome">{state[0]}</div></article>})}</div></section>
