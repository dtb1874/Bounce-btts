...
export default function CanonicalLeagueTable({ standings, seasonLabel, gameweek, entryFee, fixtures, predictions, gameweeks, adjustments }: Props) {
  ...
  return <section className={`${styles.leaguePage} uiFoundationStatCentre`} data-ui-foundation-view="table" onClick={() => setExpandedPlayer(null)}>
    <div className={styles.heading}>
      <div>
        <span>SEASON {seasonLabel} · {gameweek ? `GAMEWEEK ${gameweek.number}` : ""} · EST 2024</span>
        <h2>Stat Centre</h2>
        <p>League table, season statistics and player trends. S-N = score–nil +1. Ties: fewest 0–0 results, most BTTS wins, then alphabetical.</p>
      </div>
    ...
    <div className={`${styles.panel} ${styles.table} ${styles.fullLeagueTable} ${styles.enhancedTableShell} ${styles.leagueTableFirst} uiFoundationStatCentreTable`}>
    ...
    <section className={`${publicStyles.publicPanel} ${publicStyles.statPanel} uiFoundationStatCentreStats`}>
    ...
    <section className={`${publicStyles.publicPanel} uiFoundationStatCentrePlayers`}>
