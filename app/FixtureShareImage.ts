...
function scoreLabel(pick:FixtureSharePick){const status=pick.status??"";if(pick.homeScore==null||pick.awayScore==null)return "";if(liveStatuses.has(status))return `${pick.homeScore}–${pick.awayScore} · ${pick.elapsed!=null?`${pick.elapsed}′`:"LIVE"}`;if(status==="HT")return `${pick.homeScore}–${pick.awayScore} · HT`;if(finishedStatuses.has(status))return `${pick.homeScore}–${pick.awayScore} · ${status}`;return `${pick.homeScore}–${pick.awayScore}`}
function pickState(pick:FixtureSharePick):{text:string;state:PickState}{const finished=finishedStatuses.has(pick.status??"");const home=pick.homeScore,away=pick.awayScore;if(home==null||away==null)return {text:"PENDING",state:"pending"};if(home===0&&away===0)return {text:"0-0  -1",state:"zeroZero"};if(home>0&&away>0)return {text:finished?"WON  +3":"WON",state:"win"};return {text:finished?"SCORE-NIL  +1":"SCORE-NIL LIVE",state:"scoreNil"}}
