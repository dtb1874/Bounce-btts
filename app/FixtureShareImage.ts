...
function pickState(pick:FixtureSharePick):{text:string;state:PickState}{const finished=finishedStatuses.has(pick.status??"");const home=pick.homeScore,away=pick.awayScore;if(home==null||away==null)return {text:"PENDING",state:"pending"};if(home===0&&away===0)return {text:finished?"0-0  -1":"0-0 LIVE",state:"zeroZero"};if(home>0&&away>0)return {text:finished?"WON  +3":"WON",state:"win"};return {text:finished?"SCORE-NIL  +1":"SCORE-NIL LIVE",state:"scoreNil"}}
