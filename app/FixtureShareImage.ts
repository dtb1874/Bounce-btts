...
const bettingCompetitionOrder:Array<[RegExp,number]>=[[/\benglish premier league\b/i,10],[/\benglish championship\b/i,20],[/\benglish league one\b/i,30],[/\benglish league two\b/i,40],[/\bscottish premiership\b/i,50],[/\bscottish championship\b/i,60],[/\bscottish league one\b/i,70],[/\bscottish league two\b/i,80],[/\bnational league\b/i,90],[/\bnational league north\b/i,100],[/\bnational league south\b/i,110]];
...
function rank(name:string){return bettingCompetitionOrder.find(([pattern])=>pattern.test(name))?.[1]??500}
function sorted(picks:FixtureSharePick[]){return [...picks].sort((a,b)=>new Date(a.kickoffAt).getTime()-new Date(b.kickoffAt).getTime()||rank(a.competition)-rank(b.competition)||a.competition.localeCompare(b.competition,"en-GB",{sensitivity:"base"})||a.homeTeam.localeCompare(b.homeTeam,"en-GB",{sensitivity:"base"})||a.awayTeam.localeCompare(b.awayTeam,"en-GB",{sensitivity:"base"}))}
...
function gcd(a:bigint,b:bigint){while(b!==0n){const t=b;b=a%b;a=t}return a<0n?-a:a}
function combinedFractionalOdds(picks:FixtureSharePick[]){
  if(!picks.length)return null;
  let decimalNumerator=1n,decimalDenominator=1n;
  for(const pick of picks){
    const match=pick.odds?.trim().match(/^(\d+)\s*\/\s*(\d+)$/);
    if(!match)return null;
    const numerator=BigInt(match[1]),denominator=BigInt(match[2]);
    if(denominator===0n)return null;
    decimalNumerator*=numerator+denominator;
    decimalDenominator*=denominator;
    const common=gcd(decimalNumerator,decimalDenominator);
    decimalNumerator/=common;decimalDenominator/=common;
  }
  let numerator=decimalNumerator-decimalDenominator,denominator=decimalDenominator;
  const common=gcd(numerator,denominator);numerator/=common;denominator/=common;
