...
function buildFrames({ profiles, gameweeks, predictions, adjustments }: Omit<Props, "seasonLabel" | "inline">): RaceFrame[] {
  ...
  return scoredWeeks.map((gw) => {
    for (const player of players) {
      ...
      if (prediction?.points_awarded != null) { total.points += prediction.points_awarded; if (prediction.points_awarded === 3) total.wins += 1; if (prediction.points_awarded === -1) total.zeroZero += 1; }
      if (adjustment && !ignoreMissedAdjustment) total.points += adjustment.points;
    }
    const rows = players.map((player) => ({ id: player.id, name: player.display_name, ...totals.get(player.id)!, position: 0 })).sort((a, b) => b.points - a.points || a.zeroZero - b.zeroZero || b.wins - a.wins || a.name.localeCompare(b.name));
...
function rowPosition(frames: RaceFrame[], frameIndex: number, playerId: string) { return frames[frameIndex]?.rows.find((row) => row.id === playerId)?.position ?? 1; }
function pointsAtProgress(frames: RaceFrame[], playerId: string, progress: number): RacePoint[] { if (!frames.length) return []; const clamped = Math.max(0, Math.min(frames.length - 1, progress)); const whole = Math.floor(clamped); const frac = clamped - whole; const points: RacePoint[] = Array.from({ length: whole + 1 }, (_, index) => ({ index, position: rowPosition(frames, index, playerId) })); if (frac > 0 && whole < frames.length - 1) { const from = rowPosition(frames, whole, playerId), to = rowPosition(frames, whole + 1, playerId); points.push({ index: whole + frac, position: from + (to - from) * frac, moving: true }); } return points; }
function encodePayload(payload: SharedRacePayload) { const bytes = new TextEncoder().encode(JSON.stringify(payload)); let binary = ""; for (const byte of bytes) binary += String.fromCharCode(byte); return btoa(binary).replace(/\+/g, "-").replace(/\//g, "_").replace(/=+$/g, ""); }
...
export default function PositionRacePortal(props: Props) {
  ...
  function move(delta: number) { setPlaying(false); setProgress((value) => Math.max(0, Math.min(frames.length - 1, Math.round(value) + delta))); }
  function togglePlayer(id: string) { setSelectedIds((current) => { const next = new Set(current); if (next.has(id)) next.delete(id); else next.add(id); return next; }); }

  async function shareAnimation() {
    if (!frames.length || sharing) return;
    if (!visiblePlayers.length) { window.alert("Select at least one player before sharing the race."); return; }
    setSharing(true);
    try {
      const shareFrameCount = frames.length === 1 ? 1 : (frames.length - 1) * SHARE_STEPS_PER_GW + 1;
      await exportAnimatedShare({
        width: 1080, height: 1080, frameCount: shareFrameCount, frameDurationMs: Math.round(MS_PER_GW / SHARE_STEPS_PER_GW), finalHoldMs: 1600,
        filename: `bounce-league-race-${props.seasonLabel.replace(/[^0-9a-z]+/gi, "-")}.mp4`, title: `Bounce BTTS League Race · ${props.seasonLabel}`,
        drawFrame: (ctx, shareIndex) => {
          ...
          ctx.fillStyle = "#f3eadf"; ctx.fillRect(0, 0, 1080, 1080);
          ctx.fillStyle = "rgba(95,31,54,.045)"; ctx.beginPath(); ctx.arc(930, 110, 190, 0, Math.PI * 2); ctx.fill();
          ctx.fillStyle = "#8e5b35"; ctx.font = "900 18px Arial"; ctx.fillText(`SEASON ${props.seasonLabel}`, 54, 50);
          ctx.fillStyle = "#431326"; ctx.font = "700 48px Georgia"; ctx.fillText("League Position Race", 54, 102);
          ctx.fillStyle = "#746c66"; ctx.font = "700 17px Arial"; ctx.fillText("Bounce BTTS League · season movement by gameweek", 54, 132);
          ctx.strokeStyle = "rgba(95,31,54,.18)"; ctx.beginPath(); ctx.moveTo(54, 154); ctx.lineTo(1025, 154); ctx.stroke();
          ctx.fillStyle = "#b78d36"; ctx.font = "900 30px Arial"; ctx.textAlign = "right"; ctx.fillText(`GW ${frames[shareDisplayIndex].gameweek}`, 1025, 103); ctx.textAlign = "left";
          ...
          for (let rank = 1; rank <= players.length; rank++) { const y = sy(rank); ctx.strokeStyle = "rgba(95,31,54,.11)"; ctx.lineWidth = 1; ctx.beginPath(); ctx.moveTo(gx0, y); ctx.lineTo(gx1, y); ctx.stroke(); ctx.fillStyle = "#81776f"; ctx.font = "700 15px Arial"; ctx.fillText(String(rank), 68, y + 5); }
          frames.forEach((frame, index) => { ctx.fillStyle = "#81776f"; ctx.font = "700 13px Arial"; ctx.textAlign = "center"; ctx.fillText(`GW${frame.gameweek}`, sx(index), gy1 + 34); ctx.textAlign = "left"; });
          visiblePlayers.forEach((player) => { const colour = colourById.get(player.id) ?? "#b78d36"; const pts = pointsAtProgress(frames, player.id, shareProgress); ctx.strokeStyle = colour; ctx.lineWidth = 6; ctx.lineCap = "round"; ctx.lineJoin = "round"; ctx.beginPath(); pts.forEach((point, index) => { if (index === 0) ctx.moveTo(sx(point.index), sy(point.position)); else ctx.lineTo(sx(point.index), sy(point.position)); }); ctx.stroke(); const last = pts.at(-1); if (last) { ctx.fillStyle = colour; ctx.beginPath(); ctx.arc(sx(last.index), sy(last.position), 9, 0, Math.PI * 2); ctx.fill(); ctx.strokeStyle = "#f3eadf"; ctx.lineWidth = 2; ctx.stroke(); } });
          visiblePlayers.forEach((player, visibleIndex) => { const col = visibleIndex % 4, row = Math.floor(visibleIndex / 4), x = 72 + col * 252, y = 850 + row * 66; const colour = colourById.get(player.id) ?? "#b78d36"; ctx.fillStyle = colour; ctx.beginPath(); ctx.arc(x, y, 20, 0, Math.PI * 2); ctx.fill(); ctx.fillStyle = "#271d22"; ctx.font = "900 12px Arial"; ctx.textAlign = "center"; ctx.fillText(initials(player.name), x, y + 4); ctx.textAlign = "left"; ctx.fillStyle = "#34252c"; ctx.font = "800 16px Arial"; ctx.fillText(player.name, x + 30, y + 6); });
          ctx.strokeStyle = "rgba(95,31,54,.16)"; ctx.beginPath(); ctx.moveTo(54, 995); ctx.lineTo(1025, 995); ctx.stroke(); ctx.fillStyle = "#5f1f36"; ctx.font = "900 14px Arial"; ctx.fillText("BOUNCE BTTS LEAGUE", 54, 1028); ctx.fillStyle = "#746c66"; ctx.font = "700 13px Arial"; ctx.textAlign = "right"; ctx.fillText("1 = top · official Bounce tie-break rules", 1025, 1028); ctx.textAlign = "left";
        },
      });
    } catch (error) { window.alert(error instanceof Error ? error.message : "Could not create animated share."); } finally { setSharing(false); }
  }

  async function shareInteractive() {
    if (!frames.length) return;
    const payload: SharedRacePayload = { v: 1, seasonLabel: props.seasonLabel, players: players.map((player) => ({ id: player.id, name: player.name, colour: colourById.get(player.id) ?? "#d8b76f" })), frames: frames.map((frame) => ({ gameweek: frame.gameweek, positions: Object.fromEntries(frame.rows.map((row) => [row.id, row.position])) })), selected: visiblePlayers.length ? visiblePlayers.map((player) => player.id) : players.map((player) => player.id) };
    const url = `${window.location.origin}/race-share?d=${encodePayload(payload)}`;
    try { if (navigator.share) await navigator.share({ title: "Bounce BTTS League Position Race", text: `Interactive league race · ${props.seasonLabel}`, url }); else if (navigator.clipboard) { await navigator.clipboard.writeText(url); window.alert("Interactive race link copied."); } else window.prompt("Copy this interactive race link", url); }
