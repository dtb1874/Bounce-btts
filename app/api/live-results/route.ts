...
export async function GET(request: NextRequest) {
  ...
  if (!fixtureIds.length) return NextResponse.json({ updated: 0, providerFixtures: 0, live: 0, finished: 0 });
  ...
  if (!providerIds.length) return NextResponse.json({ updated: 0, providerFixtures: 0, live: 0, finished: 0 });
  ...
  for (const batch of batches) {
    ...
    for (const item of payload.response ?? []) {
      ...
      if (isFinished && home != null && away != null) {
        await admin.from("predictions").update({ points_awarded: pointsForScore(home, away) }).eq("fixture_id", localId);
      }
    }
  }

  return NextResponse.json({ updated, providerFixtures: providerIds.length, live, finished: finishedCount, refreshedAt: now });
}
