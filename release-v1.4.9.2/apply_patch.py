...
# Logged-in League Table: use the exact compact stats and expandable player-tendency presentation from the public table.
public_styles_import = 'import publicStyles from "./PublicLeagueTable.module.css";'
styles_import = 'import styles from "./release.module.css";'
if public_styles_import not in league:
    if styles_import not in league:
        raise SystemExit("League table public styles import anchor not found")
    league = league.replace(styles_import, styles_import + '\n' + public_styles_import, 1)

old_league_signature = 'function LeagueTable({standings,seasonLabel,gameweek,entryFee,fixtures,predictions}:{standings:Standing[];seasonLabel:string;gameweek:Gameweek|null;entryFee:number;fixtures:Fixture[];predictions:Prediction[]}){\n  const prizePot=standings.length*entryFee;'
new_league_signature = 'function LeagueTable({standings,seasonLabel,gameweek,entryFee,fixtures,predictions}:{standings:Standing[];seasonLabel:string;gameweek:Gameweek|null;entryFee:number;fixtures:Fixture[];predictions:Prediction[]}){\n  const [expandedPlayer,setExpandedPlayer]=useState<string|null>(null);\n  const prizePot=standings.length*entryFee;'
if new_league_signature not in league:
    if old_league_signature not in league:
        raise SystemExit("LeagueTable state anchor not found")
    league = league.replace(old_league_signature, new_league_signature, 1)

old_league_render = '''  return <section className={styles.leaguePage}><Heading eyebrow={`SEASON ${seasonLabel} · ${gameweek?`GAMEWEEK ${gameweek.number}`:""} · EST 2024`} title="League Table" actions={<span className={styles.shareInline}><ShareTableButton rows={standings} seasonLabel={seasonLabel} gameweekNumber={gameweek?.number??null} prizePot={prizePot}/></span>}><p>S-N = score–nil +1. Ties: fewest 0–0 results, most BTTS wins, then alphabetical.</p></Heading><div className={`${styles.panel} ${styles.table} ${styles.fullLeagueTable} ${styles.enhancedTableShell} ${styles.leagueTableFirst}`}><div className={`${styles.tableRow} ${styles.header}`}><span>POS</span><span>PLAYER</span><span>P</span><span>W</span><span>S-N</span><span>0-0</span><span>PTS</span></div>{standings.map((r,i)=><div key={r.id} className={`${styles.tableRow} ${i===0?styles.leader:""} ${i<3?styles.tableRowTopThree:""}`}><span className={styles.positionCell}>{i===0?"🏆":i+1}</span><strong>{r.name}</strong><span>{r.played}</span><span>{r.wins}</span><span>{r.oneSided}</span><span>{r.zeroZeroCount}</span><b>{r.points}</b></div>)}</div><div className={styles.leagueStatsBand}><article><span>LEAGUE LEADER</span><strong>{standings[0]?.name??"—"}</strong><small>{standings[0]?`${standings[0].points} pts`:"No scores yet"}</small></article><article><span>SEASON POT</span><strong>£{prizePot.toFixed(0)}</strong><small>{standings.length} active players</small></article><article><span>GOALS IN PICKS</span><strong>{leagueGoals}</strong><small>Finished selected fixtures</small></article><article><span>FINISHED PICKS</span><strong>{completed.length}</strong><small>{predictions.length} selections recorded</small></article></div><div className={styles.seasonFacts}>{facts.map(f=><article key={f.label}><span>{f.label}</span><strong>{f.value}</strong><small>{f.detail}</small></article>)}</div><div className={`${styles.panel} ${styles.playerInsightPanel}`}><div className={styles.panelHeading}><div><div className={styles.title}>PLAYER TENDENCIES</div><h3>Season selection stats</h3></div></div><div className={styles.playerInsightGrid}>{insights.map(r=><article key={r.id} className={styles.playerInsightCard}><header><strong>{r.name}</strong><span>{r.points} pts</span></header><div><span>TOTAL GOALS</span><b>{r.goals}</b></div><div><span>AVG GOALS / PICK</span><b>{r.average?r.average.toFixed(1):"—"}</b></div><div><span>RESULT SPLIT</span><b>{r.home}H · {r.draws}D · {r.away}A</b></div><footer><span>Most picked competition</span><strong>{r.favourite}</strong></footer></article>)}</div></div></section>'''
new_league_render = '''  return <section className={styles.leaguePage} onClick={()=>setExpandedPlayer(null)}><Heading eyebrow={`SEASON ${seasonLabel} · ${gameweek?`GAMEWEEK ${gameweek.number}`:""} · EST 2024`} title="League Table" actions={<span className={styles.shareInline}><ShareTableButton rows={standings} seasonLabel={seasonLabel} gameweekNumber={gameweek?.number??null} prizePot={prizePot}/></span>}><p>S-N = score–nil +1. Ties: fewest 0–0 results, most BTTS wins, then alphabetical.</p></Heading><div className={`${styles.panel} ${styles.table} ${styles.fullLeagueTable} ${styles.enhancedTableShell} ${styles.leagueTableFirst}`}><div className={`${styles.tableRow} ${styles.header}`}><span>POS</span><span>PLAYER</span><span>P</span><span>W</span><span>S-N</span><span>0-0</span><span>PTS</span></div>{standings.map((r,i)=><div key={r.id} className={`${styles.tableRow} ${i===0?styles.leader:""} ${i<3?styles.tableRowTopThree:""}`}><span className={styles.positionCell}>{i===0?"🏆":i+1}</span><strong>{r.name}</strong><span>{r.played}</span><span>{r.wins}</span><span>{r.oneSided}</span><span>{r.zeroZeroCount}</span><b>{r.points}</b></div>)}</div><section className={`${publicStyles.publicPanel} ${publicStyles.statPanel}`}><div className={publicStyles.sectionHeading}><div><span>SEASON SNAPSHOT</span><h3>League Stats</h3></div></div><div className={publicStyles.statCluster}><div className={publicStyles.statItem}><span>LEAGUE LEADER</span><strong>{standings[0]?.name??"—"}</strong><small>{standings[0]?`${standings[0].points} pts`:"No scores yet"}</small></div><div className={publicStyles.statItem}><span>SEASON POT</span><strong>£{prizePot.toFixed(0)}</strong><small>{standings.length} active players</small></div><div className={publicStyles.statItem}><span>GOALS IN PICKS</span><strong>{leagueGoals}</strong><small>Finished selected fixtures</small></div><div className={publicStyles.statItem}><span>FINISHED PICKS</span><strong>{completed.length}</strong><small>{predictions.length} selections recorded</small></div>{facts.map(f=><div className={publicStyles.statItem} key={f.label}><span>{f.label}</span><strong>{f.value}</strong><small>{f.detail}</small></div>)}</div></section><section className={publicStyles.publicPanel}><div className={publicStyles.sectionHeading}><div><span>SEASON SELECTION PROFILE</span><h3>Player Tendencies</h3></div></div><div className={publicStyles.playerList}>{insights.map(r=>{const open=expandedPlayer===r.id;return <article className={`${publicStyles.playerCard} ${open?publicStyles.playerCardOpen:""}`} key={r.id} onClick={event=>event.stopPropagation()}><button type="button" className={publicStyles.playerSummary} aria-expanded={open} onClick={()=>setExpandedPlayer(open?null:r.id)}><strong>{r.name}</strong><span>{r.points} pts</span><b>{open?"−":"+"}</b></button>{open&&<div className={publicStyles.playerDetails}><div><span>TOTAL GOALS</span><b>{r.goals}</b></div><div><span>AVG GOALS / PICK</span><b>{r.average?r.average.toFixed(1):"—"}</b></div><div><span>RESULT SPLIT</span><b>{r.home}H · {r.draws}D · {r.away}A</b></div><div className={publicStyles.playerCompetition}><span>MOST PICKED COMPETITION</span><b>{r.favourite}</b></div></div>}</article>})}</div></section></section>'''
if new_league_render not in league:
    if old_league_render not in league:
        raise SystemExit("Logged-in league stats renderer anchor not found")
    league = league.replace(old_league_render, new_league_render, 1)
...
print("Applied v1.4.9.2 gameweek/admin/crest + compact dashboard + public-style league stats hotfix")
